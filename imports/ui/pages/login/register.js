...
class Register extends React.Component {
  ...
  render() {
    const updateUsernameUniqueState = error => error ?
                this.setState({ userNameUniqueError: true })
                : this.setState({ userNameUniqueError: false });
    const updateConfirmPasswordState = error => error ?
                this.setState({ passwordConfirmError: true })
                : this.setState({ passwordConfirmError: false });
    const registerAction = (isFormValid, username, password) => isFormValid ? Accounts.createUser({
      username,
      password,
    }, ((e) => {
